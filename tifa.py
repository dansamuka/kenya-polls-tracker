...
def discover_sources(timeout: int = 25) -> List[Dict[str, Optional[str]]]:
    """Discover TIFA poll pages and PDFs from the official polls page."""
    ...
    for link in soup.select("a[href]"):
        ...
        if not href:
            continue
        absolute_url = urljoin(TIFA_POLLS_URL, href)
        ...
        page_url = TIFA_POLLS_URL if pdf_url else absolute_url
        ...
        sources.append(
            {
                ...
                "title": title[:240] or "TIFA Research poll release",
                "page_url": page_url,
                "pdf_url": pdf_url,
